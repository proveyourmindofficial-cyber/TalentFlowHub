...
export class TeamsService {
  ...
  async createOnlineMeeting(options: TeamsOnlineMeetingOptions): Promise<TeamsOnlineMeeting | null> {
    ...
    try {
      // Use Microsoft Graph API to create a real Teams meeting
      const meetingRequest = {
        subject: options.subject,
        startDateTime: options.startDateTime,
        endDateTime: options.endDateTime,
        joinWebUrl: '', // This will be generated by Teams
      };

      // Create the online meeting using the user endpoint (with application permissions)
      // Use the organizer's email as the user context
      const meeting = await this.graphClient
        .api(`/users/${options.organizerEmail}/onlineMeetings`)
        .post(meetingRequest);

      console.log(`✅ Teams meeting created successfully: ${meeting.joinWebUrl}`);

      return {
        id: meeting.id,
        joinUrl: meeting.joinWebUrl,
        meetingId: meeting.onlineMeetingId || meeting.id,
        subject: options.subject,
        startDateTime: options.startDateTime,
        endDateTime: options.endDateTime,
      ...
      console.error('❌ Error creating Teams meeting via Graph API:', error);
      console.log('📅 Falling back to manual Teams meeting link generation...');
      
      // Fallback to the working URL generation method
